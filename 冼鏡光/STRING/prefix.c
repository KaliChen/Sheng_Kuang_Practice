...
void      prefixinitial(void);      /* stack initialization     */
...
int       prefix_stack_top(void);    /* return type of top item  */
...
void  prefix(void)
{
     ...
     for (p = s; *p != '\0'; p++)  /* for each character  */
          if (is_opr(*p))     /* is is an operator ?      */
               ...
               while (prefix_stack_top() == OPERAND) { 
...
/* ------------------------------------------------------ */
/* FUNCTION  prefix_stack_top :                                  */
/*    Returns the type of the top-most item in stack.     */
/* ------------------------------------------------------ */

int  prefix_stack_top(void)
{
